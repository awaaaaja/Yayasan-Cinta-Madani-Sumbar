      ...
      <div class="rounded-2xl border border-[var(--color-sand)] bg-white p-6 shadow-sm">
        <form class="space-y-4" @submit.prevent="handleLogin">
          <div class="space-y-1.5">
            <label class="block text-small font-semibold text-[var(--color-bark)]">Email</label>
            <input
              v-model="email"
              type="email"
              autocomplete="email"
          ...
          <div class="space-y-1.5">
            <label class="block text-small font-semibold text-[var(--color-bark)]">Password</label>
            <input
              v-model="password"
              type="password"
