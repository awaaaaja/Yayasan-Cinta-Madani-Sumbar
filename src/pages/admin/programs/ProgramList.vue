...
import { listPrograms, deleteProgram, duplicateProgram } from '@/services/admin/programs.admin.service';
import type { Program } from '@/types';

document.title = 'Manajemen Program';
...
async function fetchItems() {
  loading.value = true;
  try {
    const result = await listPrograms({ page: page.value, pageSize, search: search.value, status: statusFilter.value });
...
function handleRowClick(row: Record<string, unknown>) { router.push(`/admin/programs/${row.id}/edit`); }
...
async function handleDuplicate(id: string) {
  try {
    ...
    router.push(`/admin/programs/${dup.id}/edit`);
  } catch (e) { console.error(e); }
}
...
<template>
  <div class="space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <h1 class="text-2xl font-bold text-[var(--color-bark)]">Program</h1>
      <button
        class="inline-flex items-center gap-2 rounded-lg bg-[var(--color-forest)] px-4 py-2 text-small font-medium text-white hover:bg-[var(--color-moss)]"
        @click="router.push('/admin/programs/create')"
      >
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
      <div class="relative flex-1">
        ...
        <input type="text" placeholder="Cari program..." class="w-full rounded-lg border border-[var(--color-sand)] bg-white py-2 pl-10 pr-4 text-body text-[var(--color-bark)] placeholder-[var(--color-moss)]/50 focus:border-[var(--color-forest)] focus:outline-none focus:ring-2 focus:ring-[var(--color-forest)]/20" @input="onSearch(($event.target as HTMLInputElement).value)" />
      </div>
    ...
    <div class="hidden rounded-xl border border-[var(--color-sand)] bg-white lg:block">
      <DataTable :columns="columns" :rows="items" :loading="loading" empty-message="Belum ada program" :selected-keys="[]" @row-click="handleRowClick">
        ...
        <template #cell-featured="{ value }">
          ...
          <span v-else class="text-xs text-[var(--color-moss)]">Tidak</span>
        </template>
        <template #actions="{ row }">
          <div class="flex items-center justify-end gap-1">
            <button class="rounded p-1.5 text-[var(--color-moss)] hover:bg-[var(--color-cream)]" @click.stop="router.push(`/admin/programs/${row.id}/edit`)">
    ...
    <div class="space-y-3 lg:hidden">
      ...
      <div v-else-if="items.length === 0" class="py-12 text-center text-[var(--color-moss)]">Belum ada program</div>
      <MobileDataCard v-for="item in items" :key="item.id" :item="item" :fields="mobileFields" status-field="status" @click="router.push(`/admin/programs/${item.id}/edit`)" @edit="router.push(`/admin/programs/${item.id}/edit`)" @delete="confirmDelete(item.id)" />
    </div>
