...
import { listEvents, deleteEvent, duplicateEvent } from '@/services/admin/events.admin.service';
import type { Event } from '@/types';

document.title = 'Manajemen Event';
...
async function fetchItems() {
  loading.value = true;
  try {
    const result = await listEvents({ page: page.value, pageSize, search: search.value, status: statusFilter.value });
...
function handleRowClick(row: Record<string, unknown>) { router.push(`/admin/events/${row.id}/edit`); }
function confirmDelete(id: string) { deleteId.value = id; showDeleteDialog.value = true; }

async function handleDelete() {
  if (!deleteId.value) return;
  try { await deleteEvent(deleteId.value); showDeleteDialog.value = false; deleteId.value = null; fetchItems(); } catch (e) { console.error(e); }
}

async function handleDuplicate(id: string) {
  try { const dup = await duplicateEvent(id); router.push(`/admin/events/${dup.id}/edit`); } catch (e) { console.error(e); }
}
...
<template>
  <div class="space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <h1 class="text-2xl font-bold text-[var(--color-bark)]">Event</h1>
      <button class="inline-flex items-center gap-2 rounded-lg bg-[var(--color-forest)] px-4 py-2 text-small font-medium text-white hover:bg-[var(--color-moss)]" @click="router.push('/admin/events/create')">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
      <div class="relative flex-1">
        ...
        <input type="text" placeholder="Cari event..." class="w-full rounded-lg border border-[var(--color-sand)] bg-white py-2 pl-10 pr-4 text-body text-[var(--color-bark)] placeholder-[var(--color-moss)]/50 focus:border-[var(--color-forest)] focus:outline-none focus:ring-2 focus:ring-[var(--color-forest)]/20" @input="onSearch(($event.target as HTMLInputElement).value)" />
      </div>
    ...
    <div class="hidden rounded-xl border border-[var(--color-sand)] bg-white lg:block">
      <DataTable :columns="columns" :rows="items" :loading="loading" empty-message="Belum ada event" :selected-keys="[]" @row-click="handleRowClick">
        ...
        <template #actions="{ row }">
          <div class="flex items-center justify-end gap-1">
            <button class="rounded p-1.5 text-[var(--color-moss)] hover:bg-[var(--color-cream)]" @click.stop="router.push(`/admin/events/${row.id}/edit`)">
              <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
            </button>
            <button class="rounded p-1.5 text-[var(--color-moss)] hover:bg-[var(--color-cream)]" @click.stop="handleDuplicate(row.id as string)">
    ...
    <div class="space-y-3 lg:hidden">
      ...
      <div v-else-if="items.length === 0" class="py-12 text-center text-[var(--color-moss)]">Belum ada event</div>
      <MobileDataCard v-for="item in items" :key="item.id" :item="item" :fields="mobileFields" status-field="status" @click="router.push(`/admin/events/${item.id}/edit`)" @edit="router.push(`/admin/events/${item.id}/edit`)" @delete="confirmDelete(item.id)" />
    </div>
