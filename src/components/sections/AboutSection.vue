...
async function fetchData() {
  ...
  error.value = null;
  try {
    const { getAboutContent } = await import("@/services/homepage.service");
      ...
      <div v-else class="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20 items-center">
        ...
        >
          ...
          <h2 class="text-h1 text-text mb-6">
            {{ content?.heading || 'Pendidikan yang Membumi dan Menyentuh Hati.' }}
          </h2>

          <p class="text-body-lg text-muted mb-8 leading-relaxed">
            {{ content?.description || 'Sejak 2005, Yayasan Cinta Madani hadir sebagai lembaga pendidikan Islam yang percaya setiap anak memiliki potensi unik.' }}
          </p>

          <ArrowLink
            :to="content?.cta_url || '/profil'"
            :label="content?.cta_label || 'Baca Profil Yayasan'"
          />
        </div>
        ...
        >
          <div class="flex gap-4 md:gap-6">
            ...
            <div class="flex-[7] self-end mb-0 md:translate-y-8">
              <SmartImage
                ...
                alt="Kegiatan Yayasan Cinta Madani"
                aspect="1/1"
                rounded="var(--radius-lg)"
              />
            </div>
          ...
          >
            <p class="text-editorial-number !text-2xl !leading-none">2005</p>
