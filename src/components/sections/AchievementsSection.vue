...
async function fetchData() {
  ...
  error.value = null;
  try {
    const { listAchievements } = await import("@/services/achievements.service");
    const all = await listAchievements({ featuredOnly: true });
    achievements.value = all.slice(0, 4);
  } catch (e) {
  ...
  >
    <div class="container-site">
      ...
      <div v-else>
        ...
        >
          <SectionLabel index="06" label="Prestasi" />
          <div class="flex items-end justify-between mb-10 md:mb-14">
            <h2 class="text-h1 text-text">Prestasi Siswa.</h2>
