    ...
    <div class="container-site relative z-10">
      ...
      <div v-if="isLoading" class="animate-pulse">
        ...
        <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
          <div class="md:col-span-7 aspect-[4/3] bg-white/5 rounded-[20px]" />
          <div class="md:col-span-5 grid grid-cols-2 gap-4">
            <div class="aspect-square bg-white/5 rounded-[20px]" />
            <div class="aspect-square bg-white/5 rounded-[20px]" />
          </div>
        </div>
      </div>
      ...
      <div v-else>
        ...
        >
          <SectionLabel :index="content?.eyebrow || '05'" label="Kehidupan Sekolah" class="!text-accent [&_.text-eyebrow]:!text-accent" />
        ...
        >
          <!-- Main image (7 cols) -->
          <div
            class="md:col-span-7 relative group rounded-[20px] overflow-hidden cursor-pointer"
          ...
          >
            <SmartImage
              ...
              :hover-scale="true"
            />
            <div class="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-primary-dark/70 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-400">
              <p class="text-small text-cream-100">{{ content?.captions?.[0] || 'Kehidupan di Cinta Madani' }}</p>
            </div>
          </div>

          <!-- Detail images (5 cols) -->
          <div class="md:col-span-5 grid grid-cols-2 md:grid-cols-1 gap-4">
            <div
              v-for="(img, i) in (content?.images || fallbackImages).slice(1, 3)"
              :key="i"
              class="relative group rounded-[20px] overflow-hidden cursor-pointer"
            ...
            >
              ...
              <div class="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-primary-dark/70 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-400">
                <p class="text-small text-cream-100">{{ content?.captions?.[i + 1] || 'Aktivitas' }}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        >
          <span v-for="label in ['Belajar', 'Bermain', 'Eksplorasi']" :key="label" class="glass-dark rounded-full px-4 py-2 text-caption text-cream-100/80">
            {{ label }}
          </span>
        </div>
