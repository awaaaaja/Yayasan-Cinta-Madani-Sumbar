...
async function fetchData() {
  ...
  error.value = null;
  try {
    const { listPublishedPrograms } = await import("@/services/programs.service");
    programs.value = (await listPublishedPrograms()) as Program[];
  } catch (e) {
  ...
  >
    <div class="container-site">
      ...
      <div v-else>
        ...
        >
          <SectionLabel index="04" label="Program Unggulan" />
          <div class="flex items-end justify-between mb-10 md:mb-14">
            <h2 class="text-h1 text-text">Program Unggulan.</h2>
        ...
        <div class="hidden md:grid grid-cols-2 gap-5">
          ...
          >
            ...
            <div class="relative z-10 h-full flex flex-col justify-end p-8" :class="index === 0 ? 'min-h-[400px]' : 'min-h-[240px]'">
              <span v-if="program.icon" class="text-3xl mb-3">{{ program.icon }}</span>
          ...
          >
            <div class="relative h-44">
              ...
              <div class="absolute bottom-0 left-0 right-0 p-5">
                <span v-if="program.icon" class="text-xl mb-1 block">{{ program.icon }}</span>
