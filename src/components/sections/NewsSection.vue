...
async function fetchData() {
  ...
  error.value = null;
  try {
    const { listPublishedNews } = await import("@/services/news.service");
    const result = await listPublishedNews({ limit: 4 });
    news.value = result.items as (News & { category: NewsCategory | null })[];
  } catch (e) {
  ...
  >
    <div class="container-site">
      ...
      <div v-else>
        ...
        >
          <SectionLabel index="08" label="Berita" />
          <div class="flex items-end justify-between mb-10 md:mb-14">
            <h2 class="text-h1 text-text">Berita Terkini.</h2>
