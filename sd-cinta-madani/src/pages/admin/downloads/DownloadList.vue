...
import { listDownloads, deleteDownload } from '@/services/admin/downloads.admin.service';
import type { Download } from '@/types';

document.title = 'Manajemen Download';
...
async function fetchItems() {
  loading.value = true;
  try {
    const result = await listDownloads({ page: page.value, pageSize, search: search.value, status: statusFilter.value });
...
function handleRowClick(row: Record<string, unknown>) { router.push(`/admin/downloads/${row.id}/edit`); }
function confirmDelete(id: string) { deleteId.value = id; showDeleteDialog.value = true; }

async function handleDelete() {
  if (!deleteId.value) return;
  try { await deleteDownload(deleteId.value); showDeleteDialog.value = false; deleteId.value = null; fetchItems(); } catch (e) { console.error(e); }
}
...
<template>
  <div class="space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <h1 class="text-2xl font-bold text-[var(--color-bark)]">Download</h1>
      <button class="inline-flex items-center gap-2 rounded-lg bg-[var(--color-forest)] px-4 py-2 text-small font-medium text-white hover:bg-[var(--color-moss)]" @click="router.push('/admin/downloads/create')">
    ...
    <div class="hidden rounded-xl border border-[var(--color-sand)] bg-white lg:block">
      <DataTable :columns="columns" :rows="items" :loading="loading" empty-message="Belum ada file" :selected-keys="[]" @row-click="handleRowClick">
        ...
        <template #actions="{ row }">
          <div class="flex items-center justify-end gap-1">
            <button class="rounded p-1.5 text-[var(--color-moss)] hover:bg-[var(--color-cream)]" @click.stop="router.push(`/admin/downloads/${row.id}/edit`)">
    ...
    <div class="space-y-3 lg:hidden">
      ...
      <div v-else-if="items.length === 0" class="py-12 text-center text-[var(--color-moss)]">Belum ada file</div>
      <MobileDataCard v-for="item in items" :key="item.id" :item="item" :fields="mobileFields" status-field="status" @click="router.push(`/admin/downloads/${item.id}/edit`)" @edit="router.push(`/admin/downloads/${item.id}/edit`)" @delete="confirmDelete(item.id)" />
    </div>
